...
export async function getPropertyById(sb: SupabaseClient, id: string) {
  return sb.from("properties").select("*").eq("id", id).single();
}
...
export async function createProperty(
  ...
  return sb.from("properties").insert(payload).select().single();
}
...
export async function updateProperty(
  ...
  return sb.from("properties").update(patch).eq("id", id).select().single();
}
